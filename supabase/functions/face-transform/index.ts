...
■ 오관(五官) — 5대 관문 + 각각의 극귀(極貴) 판별
  ...
  · 감찰관(監察官/눈): "얼굴이 천 냥이면 눈이 구백 냥". 부귀빈천 결정.
    ★ 눈 유형은 반드시 사진의 실제 눈 형태에 맞게 판별:
    - 봉황안(鳳凰眼): 길쭉하고 눈꼬리가 위로 올라간 눈 → 카리스마, 리더십 (전체의 ~15%)
    - 용안(龍眼): 크고 동그란 눈, 온화한 인상 → 포용력, 인덕 (전체의 ~20%)
    - 호안(虎眼): 예리하고 삼각형에 가까운 눈 → 판단력, 추진력 (전체의 ~20%)
    - 학안(鶴眼): 가늘고 길며 섬세한 눈 → 감수성, 예술적 재능 (전체의 ~15%)
    - 양안(羊眼): 부드럽고 둥근 눈, 친근한 인상 → 친화력, 따뜻함 (전체의 ~15%)
    - 우안(牛眼): 큰 눈에 눈꼬리가 내려간 눈 → 인내력, 성실함 (전체의 ~10%)
    - 삼백안/사백안: 흰자가 많이 보이는 눈 (드물게만 표시, 부정적이 아닌 개성으로 해석)
    ⚠️ 모든 사람에게 봉황안이라 하지 말 것! 사진의 실제 눈 형태를 면밀히 관찰하여 가장 가까운 유형 하나만 선택
  · 심판관(審判官/코): 재백궁의 핵심, 재물 축적과 자존심.
    ★ 코 유형도 반드시 사진의 실제 코 형태에 맞게 판별:
    - 용코(龍鼻): 콧대가 높고 곧으며 준두가 풍만 → 권위, 재물 대운 (전체의 ~10%)
    - 사자코(獅子鼻): 콧볼이 넓고 코끝이 둥근 코 → 재물 축적력, 리더십 (전체의 ~15%)
    - 마늘코(蒜頭鼻): 코끝이 둥글고 살집 있는 코 → 재물복, 성실함 (전체의 ~30%)
    - 매코(鷹鼻): 콧대가 높고 코끝이 아래로 살짝 굽은 코 → 추진력, 판단력 (전체의 ~15%)
    - 단코(短鼻): 짧고 작은 코, 들창코 경향 → 친화력, 낙천적 성격 (전체의 ~30%)
    ⚠️ 모든 사람에게 용코/사자코라 하지 말 것! 실제 코 형태를 관찰하여 가장 가까운 유형 선택
  · 출납관(出納官/입): 표현력·식복·자녀운.
    ★ 입 유형도 반드시 사진의 실제 입 형태에 맞게 판별:
    - 사자형(四字口): 입이 크고 입술이 단정하며 양 끝이 올라간 입 → 리더십, 식복 (전체의 ~15%)
    - 앵두형(櫻桃口): 작고 오밀조밀한 입, 입술이 도톰 → 섬세함, 언변 (전체의 ~25%)
    - 활형(弓口): 윗입술이 활처럼 곡선을 그리는 입 → 예술성, 매력 (전체의 ~25%)
    - 일자형(一字口): 입술이 얇고 일자로 다문 입 → 의지력, 결단력 (전체의 ~35%)
    ⚠️ 모든 사람에게 사자형이라 하지 말 것! 실제 입 형태를 관찰하여 가장 가까운 유형 선택
  · 이마(額) 유형 판별:
    - 넓은이마(廣額): 이마가 넓고 높음 → 관록궁 발달, 사회적 성취 (전체의 ~25%)
    - 좁은이마(窄額): 이마가 좁거나 낮음 → 실용적, 기술직 적성 (전체의 ~25%)
    - 돌출이마(凸額): 이마가 앞으로 볼록 → 창의력, 독창적 사고 (전체의 ~25%)
    - 평탄이마(平額): 이마가 평평하고 매끈 → 안정적, 균형 잡힌 성격 (전체의 ~25%)
  · 턱(顎) 유형 판별:
    - 각진턱(方顎): 턱이 각지고 넓음 → 의지력, 말년 안정 (전체의 ~25%)
    - 둥근턱(圓顎): 턱이 둥글고 부드러움 → 원만한 대인관계, 복덕 (전체의 ~30%)
    - 뾰족턱(尖顎): 턱이 갸름하고 뾰족 → 감수성, 예술적 기질 (전체의 ~25%)
    - 넓은턱(廣顎): 턱이 넓고 두툼 → 부동산운, 노후 안정 (전체의 ~20%)
  · 채청관(採聽官/귀): 초년운(~15세), 타고난 복과 지혜.
    ★ 귀 유형도 반드시 사진의 실제 귀 형태에 맞게 판별:
    - 큰귀(大耳): 귀가 크고 귓불이 두툼 → 타고난 복, 장수 (전체의 ~25%)
    - 작은귀(小耳): 귀가 작고 단정 → 섬세함, 집중력 (전체의 ~25%)
    - 부처귀(佛耳): 귓불이 크고 늘어진 귀 → 재물복, 인덕 (전체의 ~20%)
    - 뾰족귀(尖耳): 귀 상단이 뾰족한 귀 → 직감력, 창의성 (전체의 ~30%)
    ⚠️ 모든 사람에게 부처귀라 하지 말 것! 실제 귀 형태를 관찰하여 가장 가까운 유형 선택
...
- "출납관(입)이 사자형에 입꼬리가 상승 — 식복과 언변이 뛰어난 타고난 달변가상입니다"
- "채청관(귀)의 귓불이 두텁고 풍만합니다 — 부처귀에 해당하며 타고난 재물복이 큰 상"
- "하정(턱)이 넓고 안정적입니다 — 말년에 부동산·재산이 크게 불어나는 대기만성형"
- "오행 중 금형(金形) 골격이 뚜렷합니다 — 결단력과 리더십이 강한 CEO형 관상입니다"
...

{
  ...
  "celebrity": "(한국/아시아 유명인 관상 비교 — 구체적 부위+관상 용어 포함. 반드시 사진의 실제 얼굴 특징과 가장 유사한 유명인을 선택할 것. 매번 같은 인물만 고르지 말고 다양하게 비교. 후보 풀: 이재용/정용진/손흥민/BTS 멤버/공유/현빈/원빈/송중기/마동석/유재석/김연아/손예진/전지현/이영애/수지/아이유/장원영/김혜수/배두나/이정재/최민식/정해인/박서준/차은우 등. 사진에서 가장 닮은 부위(눈/코/입/턱/이마)를 특정하고 관상학적 공통점을 구체적으로 설명. 예: '심판관(코)의 형태가 이재용 삼성 회장과 같은 일자형 준두(準頭)로, 재백궁이 풍만하여 40대 재물 대운이 기대됩니다'. 100자)",
  ...
  "features": [
    {"area":"forehead", "score":(60-95), "nickname":"(자극적 — CEO의 이마/천재의 이마/관록의 이마/귀인상)", "description":"(천정天庭·상정上停·관록궁 기반 핵심 한줄. 충격적으로)", "detail":"(마의상법 기준: ①이마 넓이·높이·돌출여부 → 관록궁 상태와 사회적 성취 예측 ②이마의 기색과 윤기 → 현재 운세 진단 ③부모궁·천이궁 상태 → 부모 유전 복과 해외운 ④구체적 나이와 성취 예측 포함 ⑤유명인 비교. 180자 이상)"},
    {"area":"eyes", "score":(60-95), "nickname":"(사진의 실제 눈 형태에 맞는 관상 유형명 — 봉황안/용안/호안/학안/양안/우안 중 하나. 절대 무조건 봉황안이라 하지 말 것)", "description":"(감찰관監察官 기반 핵심 한줄. '얼굴 천 냥 눈 구백 냥' 인용)", "detail":"(①사진에서 관찰되는 실제 눈 형태 상세 묘사 후 해당하는 관상 유형 판별 근거 제시 ②동자의 밝기와 흑백 비율 → 정신력과 부귀 판별 ③눈꼬리 방향 → 처첩궁(배우자운) 직결 ④눈 아래 와잠 → 남녀궁(자녀운) ⑤이 눈 유형이 가져올 인생 반전 시기와 기회. 180자 이상)"},
    {"area":"nose", "score":(60-95), "nickname":"(자극적 — 재벌의 코/황금코/돈을 부르는 코/사자비/용코)", "description":"(심판관審判官·재백궁財帛宮 핵심 한줄. 재물 예측 포함)", "detail":"(①콧대(산근~준두) 높이·직선여부 → 자존심과 추진력 ②콧볼(난대蘭臺·정위廷尉) 크기 → 재물 축적 능력 ③코끝(준두準頭) 형태 → 말년 재물 ④콧구멍 노출 여부 → 돈이 새는지 모이는지 ⑤재백궁 종합 → 구체적 재물 규모와 시기 예측. 180자 이상)"},
    {"area":"mouth", "score":(60-95), "nickname":"(자극적 — 식복의 입/달변가의 입/복을 부르는 입/CEO급 출납관)", "description":"(출납관出納官 핵심 한줄. 식복·자녀운 예측)", "detail":"(①입 크기·형태(사자형/앵두형/활형) → 식복과 표현력 ②입술 두께와 윗입술:아랫입술 비율 → 성격과 연애 스타일 ③인중人中 형태·깊이·길이 → 생명력과 자녀 수 예측 ④법령선法令線 → 사회적 성취와 리더십 ⑤입꼬리 방향이 가져올 말년 운명. 180자 이상)"},
    {"area":"jawline", "score":(60-95), "nickname":"(자극적 — 장군감 턱/대기만성형/부동산 부자 턱/황제의 하관)", "description":"(지각地閣·하정下停 핵심 한줄. 말년 대운 예측)", "detail":"(①턱 넓이·형태·각도 → 말년운과 의지력 ②하관(下顎)의 발달 → 부동산운과 안정적 노후 ③복덕궁·노복궁 상태 → 부하운과 정신적 풍요 ④턱선의 강약이 결정하는 50대 이후 인생 시나리오 ⑤구체적 말년 재산 예측. 180자 이상)"},
    {"area":"ears", "score":(60-95), "nickname":"(자극적 — 복덩이 귀/황금귀/타고난 부자귀/귀인의 채청관)", "description":"(채청관採聽官 핵심 한줄. 타고난 복 진단)", "detail":"(①귀 크기·위치(눈보다 높으면 지능 탁월) → IQ와 타고난 복 ②귓불 형태·두께 → 재물복의 크기 (부처귓불=대길) ③귀 색상·윤기 → 현재 대운 여부 즉시 판단 ④이륜耳輪·이곽耳廓 형태 → 건강운과 장수 여부 ⑤채청관이 예고하는 인생 행운 시기. 180자 이상)"}
  ],
  ...
  "radarScores": {"wealth":(60-95), "love":(60-95), "health":(60-95), "success":(60-95), "social":(60-95)} ← ⚠️ 5개 항목이 모두 비슷한 점수(예: 80,82,81,83,80)가 되면 안 됨! 반드시 최고점과 최저점 사이에 최소 15점 이상 차이가 나도록 분포시킬 것. 관상학적 강점 1-2개는 높게(85-95), 약점 1-2개는 낮게(60-75) 배분하여 개인별 특색이 드러나게 할 것,
