...

{
  ...
  "overview": {
    "_tone": "overview 작성은 2단계다. 반드시 순서대로. STEP 1 — 추출: 각 상세 분석(personality.core, career.analysis 등)에서 가장 구체적인 문장 1개를 그대로 뽑아. 사주 간지가 언급된 문장 우선. 예를 들어 personality.core에 '월간 무토(戊土)가 겁재로 작용하여 가까운 사람일수록 양보를 못 하고 끝까지 이기려는 성향이 있어요'가 있으면 이 문장을 뽑는다. STEP 2 — 변환: 뽑은 문장에서 사주 용어를 빼고, 일상 행동으로 바꾸고, 20자 내외로 줄여. 위 예시 → '가까운 사람한테 양보를 못 해서 사소한 거에 항상 싸움' 또는 → '치킨 부위 고르는 것도 지면 기분 나쁜 수준'. 핵심: STEP 1에서 뽑은 문장이 실제로 상세 분석에 있어야 함. 없는 말 만들면 실패. STEP 2에서 사주 용어(십신, 합충, 오행 이름)는 전부 빼. 일반인이 읽는 문장이니까. 다른 사주를 분석했을 때 personality.core가 다르면 overview.personality도 달라져야 함. 당연히. 문체: 구어체로 툭. 물음표 금지. ~해요/~합니다/~한다/~임/~됨/~하는 중/~하는 인간/~하는 타입 금지.",
    "poeticTitle": "STEP1: personality.core에서 이 사람의 가장 핵심적인 모순이나 특징이 드러나는 문장 1개 추출. STEP2: 사주 용어 빼고 8~18자로 줄여. 읽는 사람이 피식 웃거나 뜨끔하게.",
    "hookQuestion": "STEP1: poeticTitle과 연결되는 상세 분석 속 구체적 행동 묘사를 찾아. STEP2: 사주 용어 빼고 15~25자. 물음표 금지.",
    "personality": "STEP1: personality.core에서 장단점이 동시에 드러나는 문장 추출. STEP2: 15~25자. 장점과 단점이 한 문장에 같이 있어야 찔림.",
    "career": "STEP1: career.analysis에서 이 사주의 직업적 강점과 약점이 동시에 드러나는 문장 추출. STEP2: 15~25자.",
    "wealth": "STEP1: wealth.pattern에서 돈 버는 방식이나 쓰는 방식의 핵심 문장 추출. STEP2: 15~25자. 상세와 방향 반대면 실패.",
    "love": "STEP1: love에서 연애 패턴의 핵심 문장 추출. STEP2: 15~25자.",
    "health": "STEP1: health.weakPoints에서 가장 취약한 부위와 증상 문장 추출. STEP2: 사주 용어(오행 이름) 빼고 15~25자. 구체적 증상이 보여야.",
    "family": "STEP1: family에서 부모/형제와의 관계 패턴 문장 추출. STEP2: 15~25자.",
    "social": "STEP1: relationship.socialStyle에서 대인관계 모순이 드러나는 문장 추출. STEP2: 15~25자.",
    "yearly": "STEP1: yearly 올해 overview에서 올해의 가장 핵심적인 변화/주의점 문장 추출. STEP2: 15~30자.",
    "lifePeak": "STEP1: daeun.lifePeak 문장 추출. STEP2: 15~25자. 나이 고려해서 과거면 과거형.",
    "lifeDirection": "STEP1: finalWords에서 인생 방향 핵심 문장 추출. STEP2: 15~25자.",
    "hotKey": "위 항목 중 STEP1에서 뽑은 원문이 가장 강렬했던 1개의 키."
  },
