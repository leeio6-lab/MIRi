...
## 🚨 작업 순서 (반드시 지켜라)
1단계: structure, personality, career, wealth, love, health 등 상세 분석을 먼저 완성하라.
2단계: 상세 분석이 완성된 후, 각 상세 분석의 핵심 결론을 overview 톤으로 변환하라.
3단계: shareCard도 상세 분석 결론에서 도출하라.
⚠️ overview는 독립적으로 생성하는 것이 아니라, 상세 분석의 축약이다. 상세와 overview가 모순되면 전체 실패.
...
□ 🚨🚨 overview↔상세 일치 검증 (가장 중요):
  - overview.wealth를 읽고 wealth.pattern을 읽어라. 결론이 같은가? (재성 약한데 '가계부 적는 타입'이면 모순→실패)
  - overview.career를 읽고 career.analysis를 읽어라. 결론이 같은가? (관성 없는데 '시키면 잘한다'면 모순→실패)
  - overview.personality를 읽고 personality.core를 읽어라. 결론이 같은가?
  - overview.love를 읽고 love를 읽어라. 결론이 같은가?
  - overview.health를 읽고 health를 읽어라. 결론이 같은가?
  - overview.family를 읽고 family를 읽어라. 결론이 같은가?
  - 하나라도 모순이면 전체 응답 무효. overview는 상세 분석의 축약이다.
□ poeticTitle을 읽고 '이거 완전 나야' 소름이 돋는가? (밋밋하면 실패)
□ hookQuestion에 물음표(?)가 없는가? (있으면 실패)
□ 키워드만 던진 항목이 없는가? (완결된 문장이어야 함)
□ 🚨 overview에서 톤 참고 예시를 그대로 복사하지 않았는가? (참고는 톤 수준만, 내용은 이 사주 간지에서 100% 창작)
□ 🚨 같은 프롬프트로 다른 사주를 분석했을 때 비슷한 결과가 나올 수 있는가? (나올 수 있으면 실패)
...

{
  ...
  "overview": {
    "_tone": "🔥 캡처해서 카톡으로 보내고 싶은 한 줄. 톤: 친한 형이 소주잔 내려놓고 한마디 던지는 느낌. 찔리는데 웃김. ⚡ 종결 문체: 반드시 종결형으로 끝낼 것. 허용: '~임' '~됨' '~씀' '~음' '~짐' '~중' '~없음' 또는 명사로 뚝 끊기. ⛔ 금지 종결: '~한다' '~쓴다' '~간다' '~있다' '~없다' '~된다' (하다/다 체 전부 금지). ⛔ 금지 표현: ~해요/~습니다/물음표/이모지/~하는 경향/~하는 체질. 🚨 내용 규칙: 반드시 아래 상세 분석의 결론을 이 톤으로 변환. 상세와 모순되면 전체 실패. ✅ 긍정 50% + 부정 50%.",
    "poeticTitle": "이 사주 한 줄 요약 별명. 10~20자. personality.core 결론 기반. 종결형('~임/~됨/~음') 또는 명사 끊기. 복사 금지, 이 사주 구조에서 창작.",
    "hookQuestion": "poeticTitle을 뒷받침하는 구체적 행동 묘사 한 줄. 15~25자. 물음표 금지. 종결형 필수. 예시 톤 참고만: '또 새벽 3시에 퇴근하고 택시 잡는 중', '회의실에서 혼자 3개 안건 다 처리하고 나옴'",
    "personality": "personality.core의 결론을 찔리게 한 줄로. 12~22자. 종결형 필수. 톤 참고: '속으로 다 계산 끝내놓고 모르는 척함', '지면 이틀은 곱씹음', '겉은 순한데 속에 용암 가득 채운 타입임'",
    "career": "career.analysis의 결론을 한 줄로. 12~22자. 종결형 필수. 톤 참고: '혼자 하면 천재, 팀플하면 답답해서 못 버팀', '월급보다 내 이름 걸고 뭔가 하고 싶은 욕구 큼', '시키는 건 잘하는데 스스로 시작은 못함'",
    "wealth": "wealth.pattern의 결론을 한 줄로. 12~22자. 종결형 필수. 🚨 상세에서 '재성 약'이면 돈 관심 없는 방향, '편재 강'이면 한탕 방향. 정반대 쓰면 실패. 톤 참고: '돈은 벌 줄 아는데 쓸 줄도 앎', '통장에 관심이 없어서 모이지도 않는 중', '크게 벌고 크게 날리는 체질임'",
    "love": "love의 결론을 한 줄로. 12~22자. 종결형 필수. 톤 참고: '좋아하면 올인, 아니면 눈에 안 들어옴', '밀당 못하고 직진해서 차이는 패턴 반복 중', '연애하면 세상이 그 사람뿐임'",
    "health": "health의 결론을 한 줄로. 12~22자. 종결형 필수. 오행 기반. 톤 참고: '스트레스 받으면 위장부터 뒤집어짐', '여름에 유독 지치는 이유가 사주에 있었음', '머리 열 올라오면 두통으로 직행임'",
    "family": "family의 결론을 한 줄로. 12~22자. 종결형 필수. 톤 참고: '부모한테는 약한데 형제한테는 강함', '집에서 제일 속 썩이면서 제일 걱정도 많이 끼침', '효도하고 싶은 마음은 큰데 표현이 서투름'",
    "social": "relationship의 결론을 한 줄로. 12~22자. 종결형 필수. 톤 참고: '첫인상은 좋은데 깊어지면 피곤해짐', '아는 사람 200명인데 전화할 사람은 3명임', '혼자가 편한데 외로운 건 싫은 모순 덩어리임'",
    "yearly": "올해 세운 결론을 한 줄로. 12~25자. 종결형 필수. 세운+원국 근거. 톤 참고: '올해는 참을수록 나중에 터지는 구조임', '하반기에 뭔가 하나 터짐, 좋은 쪽으로', '올해 인연이 꼬이기 쉬운 해임'",
    "lifePeak": "인생 피크 한 줄. 12~22자. 종결형 필수. 나이 고려 시제. 톤 참고: '진짜 전성기는 40대부터임', '한번 터졌고 두 번째가 오는 중임', '지금이 제일 좋은 시기, 다 밀어넣을 때임'",
    "lifeDirection": "인생 방향 한 줄. 12~22자. 종결형 필수. 명령문도 OK. 톤 참고: '흐르는 대로 가야 됨, 막으면 넘침', '뜨겁게 질러야 됨, 미지근하면 꺼짐', '묵묵히 쌓아야 됨, 급하면 무너짐'",
  ...
  "_globalRule": "⛔ 전체 JSON에서 물음표(?) 사용 절대 금지. 모든 의문문 금지. overview 항목은 반드시 종결형(~임/~됨/~씀/~음/~짐/~중/명사끊기)으로 끝낼 것. '~한다/~된다/~있다/~없다/~간다' 종결 금지. 상세 분석은 ~요 체.",

  "shareCard": {
    "title": "제3자가 봐도 흥미로운 한 줄. 🚨 반드시 이 사주의 일간 오행에 맞는 자연물 비유. 木=나무/숲/바람, 火=불/촛불/태양, 土=산/대지/바위, 金=칼/보석/거울, 水=바다/강/비. 오행이 안 맞으면 실패. (톤 참고 — 木일간→'뿌리 깊은 나무, 바람에 흔들려도 안 넘어짐', 火일간→'꺼질 듯 꺼지지 않는 불씨', 水일간→'잔잔한 호수 밑에 해류가 소용돌이침') 20~35자.",
    "tagline": "personality.core와 wealth.pattern의 핵심 결론 2개를 콤마로 연결. 축약형 반말. 상세 분석에서 도출. 30~50자.",
    "hashtags": ["#사주", "#MBTI보다정확", "#소름주의", "#운명", "#MIRI사주", "나머지 1~2개는 이 사주의 고유 특성 태그로 창작"],
    "oneLineProfile": "프로필용 한 줄. 이 사주의 용신+일간에서 도출. 15~20자.",
    "vsTag": "'{일간한자}+{핵심 특성}' 형태. 3~8자. (木→甲木/乙木, 火→丙火/丁火, 土→戊土/己土, 金→庚金/辛金, 水→壬水/癸水 + 이 사주만의 특성)"
  },
