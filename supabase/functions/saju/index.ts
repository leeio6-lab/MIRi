...
□ overview에 '겉은 X 속은 Y', '~인데 ~은 못 참는', 'X하면서 Y하는 모순' 같은 이원대립 패턴이 없는가? (있으면 실패)
□ overview에 질문형('~죠?' '~맞죠?' '~있죠?' '~하고 있지 않나요?')이 없는가? (있으면 실패, 전부 단정문으로)
□ overview 각 항목이 이 사주의 십신/합충/오행에서만 나올 수 있는 구체적 내용인가? (다른 사주에도 해당되면 실패)
□ overview와 상세 분석 간 모순이 없는가?
□ poeticTitle이 구체적 행동/상황을 묘사하는가? ('모순 인간' '이중적 성격' 같은 추상 라벨이면 실패)
...

{
  ...
  "overview": {
    "_tone": "🚨 overview는 이 앱의 핵심. 읽은 사람이 소름 돋아서 즉시 캡처 → 공유하는 수준. 뻔하면 삭제. ⛔ 절대 금지 패턴: 'X하지만 Y하는' 'X인데 Y인' '겉은 X 속은 Y' '~하면서도 ~하는' '~하는 경향' '~하는 체질' '~할 수 있는' — 이런 양면대비/상담사 문체 전부 금지. 질문형(~죠? ~맞죠?) 금지. ✅ 대신 이렇게: 하나의 구체적 행동/상황을 단정적으로 때려라. '~하는 사람이다' 가 아니라 '지금 이 순간 뭘 하고 있을지'를 묘사해라. 친구가 술자리에서 '야 너 이러잖아 ㅋㅋ' 하고 때리는 톤. 사주 근거는 숨기고 행동만 보여줘라.",
    "poeticTitle": "이 사주의 킬러 별명. 읽는 순간 소름. ⛔ 금지: '겉은 X 속은 Y' 구조, '모순' '이중성' 같은 단어, 질문형. ✅ 필수: 이 사주만의 고유한 행동 패턴을 한 방에. 구체적 상황이 떠오르는 별명. (레벨 참고: '읽씹 3분 만에 후회 카톡 보내는 사람', '퇴사 각오 매일 하면서 10년째 출근 중', '화나면 말 대신 행동으로 복수하는 조용한 폭군', '인생 계획 100개인데 실행 0개인 천재 몽상가') 15~25자.",
    "hookQuestion": "poeticTitle을 보충하는 구체적 상황 묘사. ⛔ 금지: 질문형('~죠?' '~맞죠?'), 물음표 사용 금지. ✅ 필수: 단정문으로 이 사람이 실제로 했을 행동을 콕 찍는다. 읽는 순간 '어떻게 알았지' 소름. (레벨 참고: '새벽 3시에 퇴사 후 계획표 짜다가 아침에 출근하는 루틴 반복 중', '좋아하는 사람한테 먼저 연락 안 하면서 읽씹 확인은 5분마다', '회의 때 다 알면서 일부러 모르는 척하다가 나중에 혼자 속 터지는 패턴') 20~40자.",
    "personality": "이 사람의 진짜 성격을 한 방에. ⛔ '겉은 X 속은 Y' 구조 금지. ✅ 구체적 행동으로 보여줘라. (레벨: '화나면 말 안 하고 3일 뒤에 카톡 차단하는 스타일', '혼자 결정 다 해놓고 물어본 척하는 독재자', '관심 없는 척하다가 인스타 스토리는 1초 만에 확인') 20~35자.",
    "career": "직장/일에 대한 팩트폭격. 공감 100%. ⛔ '모순' 단어 금지. (레벨: '월급 받는 날만 회사 다닐 맛 나는 직업관', '능력은 임원급인데 정치력 0이라 만년 대리', '이직사이트 매일 켜놓고 지원은 안 하는 3년째') 20~35자.",
    "wealth": "돈과의 관계를 팩트로. ⛔ 'X인데 Y' 대비 구조 금지. (레벨: '월급 들어오면 3일 안에 증발하는 통장 보유자', '아끼는 건 못 해도 후회는 매번 하는 소비 패턴', '재테크 유튜브 구독만 200개, 실행 0') 20~35자.",
    "love": "연애 패턴을 정조준. 전 애인이 떠오르게. ⛔ 질문형 금지. (레벨: '3개월 불꽃 → 6개월 권태 → 이별 후 후회, 매번 이 사이클', '본인이 차놓고 상대가 잘 되면 못 참는 유형', '연락 먼저 안 하면서 안 오면 불안해서 미치는 패턴') 20~35자.",
    "health": "몸이 보내는 경고를 팩트로. ⛔ '~하는 체질' 표현 금지. (레벨: '스트레스 → 위장 직행, 야근 다음 날 속쓰림이 증거', '잠 줄여가며 버티다가 면역력 바닥나는 루틴', '운동해야 하는 거 알면서 내일부터를 365일째 반복') 20~35자.",
    "family": "가족 관계의 급소를 팩트로. ⛔ 질문형 금지. (레벨: '부모님 전화 올 때마다 3초 고민하고 결국 받는 관계', '명절마다 참다가 집에 와서 베개에 소리 지르는 패턴', '사랑하는 건 맞는데 같이 있으면 2시간이 한계') 20~35자.",
    "social": "인간관계의 민낯을 팩트로. ⛔ 'X인데 Y' 구조 금지. (레벨: '카톡 답장 3시간 걸리면서 읽씹은 못 참는 사람', '모임 나가면 신나다가 집 오는 길에 에너지 방전', '뒤에서 욕하는 거 다 알면서 웃으며 넘기는 프로 참을성') 20~35자.",
    "yearly": "올해를 단 한 줄로 예언. 읽는 순간 긴장하거나 기대하게. (톤 참고: '올 하반기, 참았던 게 터진다 — 좋든 나쁘든', '올해 당신이 내린 결정 하나가 3년을 좌우한다', '올해는 잃는 해가 아니라 비우는 해다') 20~35자.",
    "lifePeak": "인생 전체 그래프의 핵심 한 줄. 나이 고려 필수. 희망이든 경고든 강렬하게. (톤 참고: '진짜 전성기는 아직 안 왔다, 기다려라', '지금이 바닥이다, 여기서부터 올라간다', '40대에 터지는 운이니까 지금은 씨 뿌리는 시간') 20~35자.",
    "lifeDirection": "인생의 방향을 시적 명령문으로. 용신 오행 기반. 액자에 넣고 싶은 수준. (톤 참고: '남들 따라가지 마라, 네 길은 아무도 안 간 쪽에 있다', '불처럼 태워라, 중간은 네 자리가 아니다', '멈추지 마라, 네가 쉬는 동안 운이 지나간다') 20~35자.",
