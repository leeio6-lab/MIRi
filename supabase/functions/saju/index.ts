    ...
    const callAI = async (prompt: string, model: string, maxTok: number, temp: number, system: string, seed?: number) => {
      ...
      for (let attempt = 0; attempt <= 2; attempt++) {
        ...
        if (resp.status === 429 || resp.status >= 500) {
          ...
          console.warn(`[callAI] OpenAI ${resp.status}, retry ${attempt + 1}/2 after ${waitMs}ms`);
...
{
  "overallScore": number(60-88),
  "headline": "이 사주를 꿰뚫는 한 문장. 반드시 종결형으로 끝낼 것. '~사람이다', '~가진 사람이다' 금지. 짧게 끊어. 예: '꺾이지 않는 갑목, 고집이 뿌리까지', '촛불인데 속은 용광로'. 15자 내외. 일간 오행 자연물 비유 필수. 예시 복사 금지.",
...
## 인생 운세 톤 규칙 (매우 중요)
- 이미 지난 전성기를 아쉽게 표현하지 마세요. "5세에 대운이었다"처럼 유아기 전성기를 적나라하게 언급하면 안 됩니다.
- 과거 피크는 "그때 쌓은 복과 감각이 지금의 자산"이라는 톤으로 긍정적으로 연결하세요.
- 현재~미래에 올 기회를 강조하고, "앞으로의 가능성"에 초점을 맞추세요.
- lifePeriods의 초년운 점수가 낮아도 "성장 과정에서 내공을 쌓은 시기"로 표현하세요.
- 절대 비관적이거나 후회를 유발하는 톤 금지. 읽는 사람이 기분 좋아야 합니다.
