...
## 말투 가이드
- 한국어: overview 항목은 다정한 서술형(~하는 편이에요/~이 강해요/~타입이에요). 따뜻하면서 정확하게. 상세 분석(core/analysis/pattern 등 긴 본문)은 ~요 체. 일간 오행에 맞는 자연물로 비유. 예시 복사 금지.
...
serve(async (req) => {
  ...
  try {
    ...
    const lang = locale === 'ko' ? '한국어로. overview 항목은 다정한 서술형(~하는 편이에요/~이 강해요/~타입이에요)으로 따뜻하게. 상세 분석(personality.core, career.analysis 등)은 ~요 체로 친근하게.' : locale === 'ja' ? '日本語(丁寧語)で' : 'In English, warm but direct';
...
□ overview가 건방지거나 비꼬는 톤이 아닌, 따뜻하고 다정한 톤인가? (건방지면 실패)
□ 전체 JSON에서 물음표(?)가 단 하나도 없는가? overview뿐 아니라 상세분석(core/analysis/pattern)에도 '~지?' '~죠?' '~걸?' '~나?' '~까?' 없어야 한다. (하나라도 있으면 실패)
□ overview가 읽는 사람이 기분 좋으면서도 소름 돋는 정확함이 있는가? (불쾌하면 실패)
□ overview 각 항목이 이 사주의 십신/합충/오행에서만 나올 수 있는 구체적 내용인가? (다른 사주에도 해당되면 실패)
□ overview와 상세 분석 간 모순이 없는가? (overview.family가 '형제 경쟁'이면 family 상세에도 형제 경쟁 내용이 있어야 함. overview의 핵심 주장이 상세 풀이의 핵심 결론과 동일해야 한다. 톤만 다르고 결론은 같아야 함.)
...

{
  ...
  "overview": {
    "_tone": "🔥 MIRI 바이럴 핵심. 캡처→카톡/인스타 공유 목표. 톤: 오래 다닌 단골 점집 할머니가 손 잡고 다정하게 읽어주는 느낌. 따뜻하면서도 '어떻게 알았지?' 소름 돋게. ⚡ 문체: 다정한 서술형. '~하는 편이에요/~이 강해요/~타입이에요'. 🚨 핵심 규칙: 모든 항목은 이 사주의 십신/합충/오행 구조에서만 도출. 구체적 행동·경험으로 묘사. 100% 이 사주에 맞게 창작. 다른 사주에도 해당되는 범용 표현이면 실패. ⛔ 금지: 비꼬기/조롱/건방진 말투/딱딱한 반말(~함/~됨/~씀)/물음표/이모지. ✅ 긍정 60% + 현실적 조언 40%.",
    "poeticTitle": "이 사주를 한마디로. 따뜻하면서 소름 돋는 별명. 10~20자. 🚨 톤 참고만, 복사 금지. (비겁 강→'혼자서도 해내는 사람', 식상 강→'아이디어가 끊이지 않는 사람', 재성 강→'현실 감각이 남다른 사람')",
    "hookQuestion": "poeticTitle을 뒷받침하는 구체적 행동 묘사. 15~25자. 물음표 금지. 🚨 톤 참고만. (관성 강→'맡은 일은 끝까지 해내는 책임감', 식상 강→'머릿속에 늘 새로운 생각이 가득', 인성 강→'배우는 걸 멈추지 못하는 탐구심')",
    "personality": "성격 핵심. 12~25자. 🚨 톤 참고만. (비겁 강→'한번 마음먹으면 끝까지 가는 성격', 인성 강→'깊이 생각하고 신중하게 움직이는 편', 식상 강→'표현력이 풍부하고 감성이 넘치는 타입')",
    "career": "직업운 핵심. 12~25자. 🚨 톤 참고만. (관성 강→'조직 안에서 빛나는 리더형', 식상 강→'자유로운 환경에서 능력이 배가되는 편', 재성 강→'실속을 잘 챙기는 현실적 직업관')",
    "wealth": "재물운 핵심. 12~25자. 편재/정재 구분 필수. 🚨 톤 참고만. (편재 강→'큰 흐름을 잘 읽는 투자 감각', 정재 강→'차곡차곡 모으는 안정형 재물운', 재성 약→'돈보다 가치를 우선하는 스타일')",
    "love": "연애운 핵심. 12~25자. 🚨 톤 참고만. (관성 강→'이상형이 뚜렷하고 진지한 연애 스타일', 비겁 강→'주도적이고 당당한 연애를 하는 편', 식상 강→'감정 표현이 풍부한 로맨티스트')",
    "health": "건강 핵심. 12~25자. 🚨 톤 참고만. (화 과다→'열이 위로 몰리기 쉬운 체질', 수 과다→'순환과 보온에 신경 쓰면 좋아요', 토 부족→'소화기를 챙기면 전체 컨디션이 올라가요')",
    "family": "가족 핵심. 12~25자. 🚨 톤 참고만. (인성 강→'가족 안에서 든든한 버팀목 역할', 비겁 강→'형제자매와 함께 성장하는 관계', 관성 강→'가정에 질서와 안정을 가져다주는 편')",
    "social": "대인관계 핵심. 12~25자. 🚨 톤 참고만. (식상 강→'사람을 잘 끌어모으는 매력이 있어요', 비겁 강→'깊은 관계를 소중히 여기는 스타일', 인성 강→'소수의 사람과 깊이 있는 교류를 선호')",
    "yearly": "올해 한 줄. 12~28자. 세운+원국 작용 근거. 🚨 톤 참고만. (세운이 용신→'좋은 흐름을 타는 해예요', 세운이 기신→'내면을 다지는 시간이 될 해', 세운 합→'새로운 인연이 들어오는 해')",
    "lifePeak": "인생 피크. 12~25자. 나이 고려 시제. 🚨 톤 참고만. (피크 아직→'40대부터 본격적으로 꽃이 피어요', 피크 지남→'한번 빛났고 두 번째 전성기가 다가와요', 지금이 피크→'지금이 가장 좋은 시기, 자신 있게 나아가세요')",
    "lifeDirection": "인생 방향 조언. 12~25자. 🚨 톤 참고만. (용신 수→'흐름에 맡기되 중심은 잃지 마세요', 용신 화→'열정을 믿고 과감하게 나아가세요', 용신 토→'꾸준히 쌓아가면 반드시 결실이 와요')",
  ...
  "shareCard": {
    ...
    "tagline": "이 사주의 가장 자극적인 특성 2개를 콤마로 연결. 축약형 반말. 사주 구조에서 도출. 30~50자.",
