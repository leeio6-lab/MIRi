...
□ overview 각 항목을 읽고 즉시 캡처→친구에게 카톡 보내고 싶은가? (안 보내고 싶으면 실패)
□ overview에 '~하는 경향', '~할 수 있어요', '~하는 체질' 같은 상담사 말투가 없는가? (있으면 실패)
□ 전체 JSON에서 물음표(?)가 단 하나도 없는가? overview뿐 아니라 상세분석(core/analysis/pattern)에도 '~지?' '~죠?' '~걸?' '~나?' '~까?' 없어야 한다. (하나라도 있으면 실패)
□ overview에 '~하는 체질' '~하는 경향이 있어요' 같은 상담사 말투가 없는가? (있으면 실패)
□ overview 각 항목이 이 사주의 십신/합충/오행에서만 나올 수 있는 구체적 내용인가? (다른 사주에도 해당되면 실패)
□ overview와 상세 분석 간 모순이 없는가?
□ poeticTitle을 읽고 '이거 완전 나야' 소름이 돋는가? (밋밋하면 실패)
□ hookQuestion에 물음표(?)가 없는가? (있으면 실패)
□ 키워드만 던진 항목이 없는가? (반드시 25~40자 완결된 문장)
□ 프롬프트 예시(레벨 참고)를 그대로 복사하지 않았는가? (복사하면 실패, 반드시 이 사주에 맞는 창작)
□ overview 각 항목이 형용사가 아닌 구체적 행동 묘사인가? ('감성적이다' 금지, 행동으로 보여줘야)
□ shareCard.vsTag가 '{일간한자}+특성' 형태인가? shareCard.title이 제3자가 봐도 흥미로운가?
□ timeBasedInsight의 각 조언이 이 사주의 오행/십신에서 논리적으로 도출되었는가? ('물 많이 마시세요' 수준이면 실패)

{
  ...
  "overview": {
    "_tone": "🔥 이 overview가 MIRI의 생존이 걸린 핵심이다. 목표: 유저가 결과 화면 캡처 → 카톡/인스타 스토리에 '이거 소름...' 하면서 올리는 것. 톤: 20~30대가 '아 ㅋㅋㅋ 찔린다' 또는 '소름 돋는다' 하는 수준의 정확한 팩폭. 무당 느낌 아님. 친한 형/누나가 술자리에서 정곡 찌르는 느낌. 웃기면서 찔리는 게 최고. 웃기기만 하면 가볍고, 찔리기만 하면 불쾌. ⚡ 문체: 트위터/인스타 감성의 축약형 반말. '~한다/~쓴다/~간다' 같은 정식 종결어미 금지. 대신 '~함/~씀/~임/~됨' 체를 기본으로 쓰되, 자연스러우면 명사/체언으로 툭 끊어도 좋음. 핵심은 말끝을 가볍게 끊는 느낌. 나쁜 예: '하반기에 내린 결정 하나가 내년 수입을 좌우한다' / 좋은 예: '하반기에 내린 결정 하나가 내년 수입 좌우'. 나쁜 예: '새벽 3시에 옛날 사진 보다가 전 연인 인스타까지 정주행한다' / 좋은 예: '새벽 3시에 옛날 사진 보다가 전 연인 인스타까지 정주행'. 나쁜 예: '돈 쓸 때는 통 크게 쓰는데 택시비 아까워서 30분 걸어간다' / 좋은 예: '돈 쓸 땐 통 크게 쓰는데 택시비 아까워서 30분 걸어감'. 작성법: 이 사주의 십신/합충/오행에서만 나올 수 있는 구체적 행동을 묘사. '독립적임', '감성적임' 같은 형용사 금지. 행동으로 보여줘라. 나쁜 예: '감성이 풍부한 낭만주의자' / 좋은 예: '새벽에 옛날 사진 꺼내보다가 혼자 눈물 찔끔 흘림'. 나쁜 예: '리더십이 있음' / 좋은 예: '회의에서 결국 본인이 정리하고 끝내는데 공은 팀장이 가져감'. 각 항목은 읽는 사람이 '아 이거 나인데??' 하고 멈칫하는 수준이어야 함. 다른 사주에도 해당되는 말이면 실패. ⛔ 금지: ~해요/~습니다/~한다/~쓴다/~간다/~하는 체질/~하는 경향/물음표(?)/ㅋㅋ/ㅎㅎ/이모지/진지한 상담사 말투. ✅ ~함/~씀/~임/~됨 체 또는 명사로 툭 끊기. 긍정(자랑) 50% + 부정(찔림) 50%.",
    "poeticTitle": "인스타 바이오에 바로 쓸 수 있는 킬러 별명. ~함/~씀/~임 체. 이 사주의 십신/오행에서 뽑아낸 핵심을 구체적 행동으로. 예시 복사 금지, 반드시 이 사주에 맞게 창작. (레벨 참고: '월급 들어오면 3시간 만에 잔고 반토막 내는 인간', '싸움은 피하는데 뒷담은 3년 기억하는 코끼리', '모든 팀플에서 결국 혼자 ppt 만드는 1인 시스템') 15~30자.",
    "hookQuestion": "poeticTitle이 사실인 증거. 축약형 반말. 물음표 금지. 어제 실제로 한 행동 수준의 구체성. 이 사주의 십신 배치에서 논리적으로 나오는 행동이어야 함. (레벨 참고: '어제도 유튜브 알고리즘에 2시간 바침, 원래 5분만 보려 했음', '점심 뭐 먹을지 30분 고민하다 결국 어제 먹은 거 또 시킴', '카톡에 ㅇㅇ 치다가 지우고 장문 3줄로 다시 씀') 25~45자.",
    "personality": "성격 팩폭 or 자랑. 축약형 반말. 형용사 금지, 구체적 행동으로. 비겁/식상/인성 배치에서 도출. (레벨 참고: '잘못 지적당하면 일단 인정, 근데 속으로 반박문 3개 작성 중', '약속 10분 전 도착해놓고 상대 1분 늦으면 짜증 폭발') 25~40자.",
    "career": "직업 팩폭 or 자랑. 축약형 반말. 관성/식상 배치에서 도출. (레벨 참고: '야근은 싫은데 퇴근 후 본인 프로젝트는 새벽까지', '이력서 매달 업데이트하면서 이직은 3년째 보류') 25~40자.",
    "wealth": "돈 팩폭 or 자랑. 축약형 반말. 재성 구조에서 도출. (레벨 참고: '적금 3개 넣으면서 배달비에 월 20만 원 소비', '비상금 100만 원은 무조건 남겨놓는 불안형 저축러') 25~40자.",
    "love": "연애 팩폭 or 자랑. 축약형 반말. 관성/재성/비겁 관계에서 도출. (레벨 참고: '썸 탈 때는 밀당 고수, 사귀면 하루 만에 올인', '좋아하는 사람 앞에서만 갑자기 말수 줄어드는 반전') 25~40자.",
    "health": "건강 팩폭 or 자랑. 축약형 반말. 오행 과다/부족에서 도출. (레벨 참고: '헬스 등록만 5번째, 근데 한번 가면 2시간 하는 극과 극', '스트레스 받으면 일단 먹고 봄, 그게 새벽 라면') 25~40자.",
    "family": "가족 팩폭 or 자랑. 축약형 반말. 인성/비겁 구조에서 도출. (레벨 참고: '부모님 잔소리에 대꾸 안 함, 방 들어가서 이불킥', '명절에 조카들 챙기다 에너지 다 빨리는 숨은 큰손') 25~40자.",
    "social": "인간관계 팩폭 or 자랑. 축약형 반말. 비겁/식상/관성에서 도출. (레벨 참고: '단톡방 읽씹 전문, 근데 1:1이면 새벽까지 상담', '모임 끝나면 집 가는 길에 오늘 한 말 복기하느라 지침') 25~40자.",
    "yearly": "올해 한 줄 예언. 축약형 반말. 세운과 원국의 구체적 작용에서 도출. (레벨 참고: '올해 상반기에 뭔가 하나 터짐, 기회인지 사고인지는 본인 선택', '하반기 결정 하나가 내년 수입 좌우') 25~40자.",
    "lifePeak": "인생 그래프 핵심. 축약형 반말. 나이 고려하여 시제. 대운 흐름 근거. (레벨 참고: '30대 후반부터 돈이 손에 잡히기 시작, 지금은 실력 쌓는 시간', '피크는 지났는데 두 번째가 50대에 옴') 25~40자.",
    "lifeDirection": "인생 방향 한 줄. 축약형 반말 또는 명령문. 액자에 넣고 싶은 급. 용신 오행에서 도출. (레벨 참고: '안전한 길은 네 길 아님, 좀 무서운 쪽으로 가', '혼자 하려고 하지 마, 네 운은 사람에게서 옴', '천천히 가는 게 네 속도, 빨리 가려 하지 마') 25~40자.",
    "hotKey": "위 10개 항목 중 이 사주에서 가장 핵심적이고 소름돋는 항목 1개의 키를 써라. (personality, career, wealth, love, health, family, social, yearly, lifePeak, lifeDirection 중 하나). '이것만큼은 딱 맞다'는 추천 표시에 쓰인다."
  },

  "_globalRule": "⛔ 전체 JSON에서 물음표(?) 사용 절대 금지. '~지?' '~죠?' '~걸?' '~나?' '~까?' '~하지 않나요?' — 모든 의문문·질문형 표현 금지. 모든 문장은 단정문(~다/~요)으로만 작성. 위반 시 전체 응답 무효.",

  "shareCard": {
    "title": "캡처해서 공유할 때 상단에 표시될 한 줄. poeticTitle과 다르게 제3자가 봐도 흥미를 느끼는 문장. 일간 오행의 자연물 비유 + 이 사주의 핵심 운명을 결합. 예시 복사 금지. (레벨 참고: '바다 위의 등대, 남들은 의지하는데 정작 본인은 외롭다', '마른 장작 위의 불씨, 한번 붙으면 전부 태운다') 20~35자.",
    "tagline": "인스타 스토리 하단 또는 카톡 공유 미리보기에 뜨는 2줄. 이 사주의 가장 자극적인 특성 2개를 콤마로 연결. 반말. (레벨 참고: '시작은 누구보다 빠른데 마무리는 남한테 넘기는 스타일, 근데 그래서 인복이 터진다') 30~50자.",
    "hashtags": ["#사주", "#MBTI보다정확", "#소름주의", "#운명", "#MIRI사주", "이 중 1~2개를 이 사주의 고유 특성에 맞는 태그로 교체 (예: '#타고난_인싸력', '#새벽감성러', '#돈복터짐')"],
    "oneLineProfile": "인스타/카톡 프로필에 바로 쓸 수 있는 한 줄 자기소개. 반말. 이 사주의 용신+일간에서 도출. 15~20자. (레벨 참고: '겁 없이 시작하고 끝은 운이 맺어주는 인생', '느리지만 한번 잡으면 절대 안 놓는 사람')",
    "vsTag": "이 사주를 MBTI처럼 태그화. '{일간한자}+{핵심 특성}' 형태. 3~8자. (레벨 참고: '壬水 자유영혼', '丁火 은근집착러', '甲木 올인전사', '己土 묵묵한 해결사')"
  },
  ...
  "timeBasedInsight": {
    "morningAdvice": "오전에 이 사주가 주의할 것 / 잘할 수 있는 것 1줄. 반말. 일간+식상/관성 구조에서 도출한 구체적 행동. 범용적 '물 많이 마시세요' 수준 금지. (레벨 참고: '오전 회의에서 먼저 말하지 마, 듣고 마지막에 정리하면 가장 현명해 보인다', '아침에 중요한 결정 내려, 오후 되면 감정이 판단을 흐린다') 30~50자.",
    "afternoonAdvice": "오후 조언 1줄. 반말. 이 사주의 오행 과다/부족 기반. (레벨 참고: '오후 3시 이후 집중력이 바닥난다, 중요한 건 오전에 끝내', '점심 먹고 나면 관계 에너지가 충전된다, 미뤄둔 연락은 오후에') 30~50자.",
    "nightAdvice": "저녁/밤 조언 1줄. 반말. 일간의 음양+인성/비겁 구조에서 도출. (레벨 참고: '밤에 감정적 결정 금지, 새벽에 보낸 카톡은 99% 후회한다', '자기 전 10분 정리 시간이 내일의 컨디션을 결정한다') 30~50자.",
    "weekdayTip": "평일에 이 사주가 써먹을 수 있는 팁 1줄. 반말. 관성/재성 구조에서 도출. 직장/학교 맥락. (레벨 참고: '수요일이 네가 가장 예민한 요일이다, 중요한 미팅은 화목에 잡아', '점심시간에 혼자 있는 시간 확보해, 그래야 오후가 버틴다') 30~50자.",
    "weekendTip": "주말 팁 1줄. 반말. 비겁/식상 구조에서 도출. (레벨 참고: '혼자 있으면 에너지 충전되는 사주다, 주말 약속 2개 이상 잡지 마', '주말에 움직여야 월요일이 편하다, 산책이라도 30분') 30~50자.",
    "seasonAdvice": {
      "spring": "봄에 이 사주의 목(木) 기운과의 관계에서 도출한 조언. 반말. 20~30자.",
      "summer": "여름에 화(火) 기운과의 관계. 반말. 20~30자.",
      "autumn": "가을에 금(金) 기운과의 관계. 반말. 20~30자.",
      "winter": "겨울에 수(水) 기운과의 관계. 반말. 20~30자."
    }
  },
...
이 사람은 ${pi.age}세입니다. 피크가 이미 지났으면 과거형("~였어요"), 아직 안 왔으면 미래형("~올 수 있어요").`;
      ...
      const [r1, r2] = await Promise.all([
        callAI(prompt1, model1, 7000, 0.7, SYSTEM_CORE),
